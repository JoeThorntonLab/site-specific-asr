...
summary_reconstruction_identity <- function(dt_list, model_pairs, model_names){
...
SR_model_comparison_list <- lapply(seq_len(ncol(sr.model.pairs)), function(i) {
  compare_ml_states(sr.model.pairs[1, i], sr.model.pairs[2, i], sr.alignment.size, SR_gaps)
})

sr.identity.summary <- summary_reconstruction_identity(SR_model_comparison_list, sr.model.pairs, sr.model.list)
...
rbd_model_comparison_list <- lapply(seq_len(ncol(rbd.model.pairs)), function(i) {
  compare_ml_states(rbd.model.pairs[1, i], rbd.model.pairs[2, i], rbd.alignment.size, RBD_gaps)
})

rbd.identity.summary <- summary_reconstruction_identity(rbd_model_comparison_list, rbd.model.pairs, rbd.model.list)
...
ha_model_comparison_list <- lapply(seq_len(ncol(ha.model.pairs)), function(i) {
  compare_ml_states(ha.model.pairs[1, i], ha.model.pairs[2, i], ha.alignment.size, HA_gaps)
})

ha.identity.summary <- summary_reconstruction_identity(rbd_model_comparison_list, ha.model.pairs, ha.model.list)
...
ggplot(ha.identity.summary[true_mismatch == "identical"], aes(x = model_a, y = model_b)) +
