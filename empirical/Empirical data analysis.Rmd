...
sr.model.comparison.list <- lapply(seq_len(ncol(sr.model.pairs)), function(i) {
  compare.ml.states(sr.model.pairs[1, i], sr.model.pairs[2, i], sr.alignment.size, sr.gaps)
})

rbd.model.comparison.list <- lapply(seq_len(ncol(rbd.model.pairs)), function(i) {
  compare.ml.states(rbd.model.pairs[1, i], rbd.model.pairs[2, i], rbd.alignment.size, rbd.gaps)
})

ha.model.comparison.list <- lapply(seq_len(ncol(ha.model.pairs)), function(i) {
...
# To determine if a site
# a threshold of posterior probability that will determine if there is ambiguity
# in assigning a single state
  ...
  for (i in 1:nrow(table)) {
    ...
    if (condition == "high_second") {
      ...
      if (second_max_val > threshold) {
        ...
        results <- c(results, colnames(table)[second_index])
...
get.all.altall <- function(system.results.path, alignment.size, condition, threshold){
  ...
  for (i in seq_along(pp_file_list)) {
    ...
    model_2_sequence_alt <- altall.seq(file.path(results.path, system.results.path, "ASR", pp_file_list[[i]]), threshold, condition)
    
    if(length(model_2_sequence_alt) == alignment.size){} else {print("error")}

    # Bind the node, site, model 1, model 2, and mismatch columns to the output data table
    alt.states.dt <- rbind(alt.states.dt, data.table(node = node_number,
                                         site = site, 
                                         alt_seq = model_2_sequence_alt))
  }
...
for (i in seq_along(sr.model.comparison.list)) {
  sr.model.comparison.list[[i]][, alt_mismatch := ifelse(.SD[[3]] == .SD[[4]] | .SD[[3]] == .SD[[8]], "identical", "disagreement")]
  sr.model.comparison.list[[i]][, true_alt_mismatch := ifelse(gap == "gap", "identical", alt_mismatch)]
}


for (i in seq_along(rbd.model.comparison.list)) {
  rbd.model.comparison.list[[i]][, alt_mismatch := ifelse(.SD[[3]] == .SD[[4]] | .SD[[3]] == .SD[[8]], "identical", "disagreement")]
  rbd.model.comparison.list[[i]][, true_alt_mismatch := ifelse(gap == "gap", "identical", alt_mismatch)]
}

for (i in seq_along(ha.model.comparison.list)) {
  ha.model.comparison.list[[i]][, alt_mismatch := ifelse(.SD[[3]] == .SD[[4]] | .SD[[3]] == .SD[[8]], "identical", "disagreement")]
  ha.model.comparison.list[[i]][, true_alt_mismatch := ifelse(gap == "gap", "identical", alt_mismatch)]
}
...
sr.plausible.summary$model_a <- factor(sr.plausible.summary$model_a, levels = sr.model.list)
sr.plausible.summary$model_b <- factor(sr.plausible.summary$model_b, levels = rev(paste0("alt_", sr.model.list)))

ggplot(sr.plausible.summary[true_alt_mismatch == "identical"], aes(x = model_b, y = model_a)) +
  geom_tile(color = "black", fill = "grey60") +
  geom_text(aes(label = round(percentage, digits = 1)), size = 8) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        aspect.ratio = 1) +
