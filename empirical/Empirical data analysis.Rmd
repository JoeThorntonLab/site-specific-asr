...
# Set paths to results and figures
empirical.path <- file.path(here(), "results/empirical")

sr.path <- file.path(empirical.path, "steroid_receptors/SR/")
rbd.path <- file.path(empirical.path, "sars_rbd/")
ha.path <- file.path(empirical.path, "hemagglutinin/")

figure.path <- file.path(here(), "figures/empirical_data/")
epistasis.figure.path <- file.path(here(), "figures/epistasis/")
...
## Among-site compositional heterogeneity

```{r load parsimony gap data}

read_gap_data <- function(node_directory, gap_file_path){

  # Extract node numbers from file names in node_directory
  node_file_list <- list.files(path = node_directory, pattern = "[0-9]{1,3}.txt")
  nodes <- gsub(".*?([0-9]+).*", "\\1",node_file_list)
  nodes <- c("site", nodes) # Add "site" as the first node
  
  # Load parsimony table and rename columns to match node numbers
  parsimony <- read.csv(gap_file_path)
  node_order <- colnames(parsimony)[-1] # Get the node order from the column names
  renamed_col <- c("site", gsub(".*?([0-9]+).*", "\\1",node_order)) # Rename the columns to match node numbers
  colnames(parsimony) <- renamed_col
  
  # Reorder columns according to node numbers and convert data from wide to long format
  parsimony <- parsimony[,nodes] # Reorder columns according to node numbers
  parsimony <- parsimony %>%
    gather("node", "state",2:(length(node_file_list)+1)) %>%
    mutate(gap = ifelse(state == "G", "gap", "no_gap")) # Convert data from wide to long format and create a gap vector
  
  gap_vector <- parsimony$gap # Extract the gap vector from the data
  
  return(gap_vector) # Return the gap vector
}

SR_node_order <- file.path("C:/Users/rmuniztrejo/Box/empirical_asr/results/empirical/steroid_receptors/SR/JTT/ASR")
SR_gap_file <- file.path("C:/Users/rmuniztrejo/Box/empirical_asr/results/empirical/misc/SR_parsimony_gaps.csv")

HA_node_order <- file.path("C:/Users/rmuniztrejo/Box/empirical_asr/results/empirical/hemagglutinin/FLU/ASR")
HA_gap_file <- file.path("C:/Users/rmuniztrejo/Box/empirical_asr/results/empirical/misc/HA_parsimony_gaps.csv")

RBD_node_order <- file.path("C:/Users/rmuniztrejo/Box/empirical_asr/results/empirical/sars_rbd/WAG/ASR")
RBD_gap_file <- file.path("C:/Users/rmuniztrejo/Box/empirical_asr/results/empirical/misc/RBD_parsimony_gaps.csv")


SR_gaps <- read_gap_data(SR_node_order, SR_gap_file)
HA_gaps <- read_gap_data(HA_node_order, HA_gap_file)
RBD_gaps <- read_gap_data(RBD_node_order, RBD_gap_file)
...
compare_ml_states <- function(model_1, model_2, alignment_size, model_path, gap_vector) {
  ...
  for (i in seq_along(node_file_list)) {
    ...
    model_1_sequence <- readLines(file.path(model_path, model_1, "ASR", node_file_list[[i]]))
    model_2_sequence <- readLines(file.path(model_path, model_2, "ASR", node_file_list[[i]]))
  ...
  # Modify column names
  setnames(PP_tibble, c("node", "site", "model_1", "model_2", "mismatch", "gaps", "true_mismatch"), 
...
self_comparisons <- t(matrix(sr.model.list, nrow = length(sr.model.list), ncol = 2))
...
SR_model_comparison_list <- lapply(seq_len(ncol(model_pairs)), function(i) {
  compare_ml_states(model_pairs[1, i], model_pairs[2, i], sr.alignment.size, sr.path, SR_gaps)
})
