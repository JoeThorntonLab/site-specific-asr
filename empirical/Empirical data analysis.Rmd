...
```{r load parsimony gap data, include=FALSE}
...
SR_node_order <- file.path(paste0(results.path, "/sr.ss.er/ASR"))
SR_gap_file <- file.path(paste0(data.path, "/sr_parsimony_gaps.csv"))

HA_node_order <- file.path(paste0(results.path, "/ha.ss.h1/ASR"))
HA_gap_file <- file.path(paste0(data.path, "/ha_parsimony_gaps.csv"))

RBD_node_order <- file.path(paste0(results.path, "/rbd.ss.sars/ASR"))
...
## Reconstruction comparison among models
...
node_file_list <- list.files(path = file.path(results.path, sr.results[1], "ASR"), pattern = "[0-9]{1,3}\\.txt")
