...
table <- fread("C:/Users/rmuniztrejo/Documents/git_repos/site-specific-asr/empirical/results/ha_sh_flu/ASR/2_PP.txt")
max_val <- max(table[1,])
    
    # find the index of the maximum value in the row
    # there could be sites with two max values, only pick the first one [1]
    max_index <- colnames(table)[which(table[1,] == max_val)[1]] 
?colnames
max(table[1,])
?fread
...
sr_alt_comparison_list <- lapply(seq_len(ncol(model_pairs)), function(i) {
  compare_alt_states(model_pairs[1, i], model_pairs[2, i], sr.alignment.size, sr.path, sr.gaps)
})
...
ha_alt_comparison_list <- lapply(seq_len(ncol(model_pairs)), function(i) {
  compare_alt_states(model_pairs[1, i], model_pairs[2, i], ha.alignment.size, ha.path, ha.gaps)
})
...
rbd_alt_comparison_list <- lapply(seq_len(ncol(model_pairs)), function(i) {
  compare_alt_states(model_pairs[1, i], model_pairs[2, i], rbd.alignment.size, rbd.path, rbd.gaps)
})
