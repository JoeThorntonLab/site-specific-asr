...
sr_model_comparison_list <- lapply(seq_len(ncol(sr.model.pairs)), function(i) {
  compare.ml.states(sr.model.pairs[1, i], sr.model.pairs[2, i], sr.alignment.size, sr.gaps)
})

rbd_model_comparison_list <- lapply(seq_len(ncol(rbd.model.pairs)), function(i) {
  compare.ml.states(rbd.model.pairs[1, i], rbd.model.pairs[2, i], rbd.alignment.size, rbd.gaps)
})

ha_model_comparison_list <- lapply(seq_len(ncol(ha.model.pairs)), function(i) {
...
sr.identity.summary <- summary.reconstruction.identity(sr_model_comparison_list, sr.model.pairs, sr.model.list)
...
rbd.identity.summary <- summary.reconstruction.identity(rbd_model_comparison_list, rbd.model.pairs, rbd.model.list)
...
ha.identity.summary <- summary.reconstruction.identity(ha_model_comparison_list, ha.model.pairs, ha.model.list)
...
altall.seq <- function(pp_table, threshold, condition) {
  ...
  for (i in 1:nrow(table)) {
    ...
    second_max_val <- as.numeric(sorted_row[1,2])
    ...
    if (condition == "high_second") {
      ...
      if (second_max_val > threshold) {
        ...
        result <- colnames(sorted_row[1,2])
      ...
      if (ratio < threshold) {
        # Add the amino acid with the second highest value to the results vector
        results <- c(results, colnames(sorted_row[1,2]))
...
abc <- merge(ha_model_comparison_list[[1]], ha.altall.seqs.list[[2]], by = c("node", "site"))

abc[, alt_mismatch := ifelse(h1 == h3 | h1 == alt_h3, "identical", "disagreement")]
abc[, true_alt_mismatch := ifelse(gap == "gap", "identical", alt_mismatch)]
