...
requiredPackages <- c("data.table", "here", "ggplot2")
...
# reconstructions in a column named "true_mismatch". The pairwise comparison is
...
summary.reconstruction.identity <- function(dt_list, model_pairs, model_names){
  ...
  for(i in 1:length(dt_list)){
    ...
    # in the true_mismatch column. Percentages are relative to the total number of
  ...
  self_comparisons <- data.table(true_mismatch = "identical", 
...
plot.pairwise.ML <- function(summarized_dt){
  ...
  plot <- ggplot(summarized_dt[true_mismatch == "identical"], aes(x = model_a, y = model_b)) +
...
for (i in seq_along(sr.model.comparison.list)) {
  ...
  sr.model.comparison.list[[i]][, true_alt_mismatch := ifelse(gap == "gap",
                                                              "identical",
                                                              alt_mismatch)]
}
...
for (i in seq_along(rbd.model.comparison.list)) {
  rbd.model.comparison.list[[i]][, alt_mismatch := ifelse(.SD[[3]] == .SD[[4]] | 
                                                            .SD[[3]] == .SD[[8]], 
                                                          "identical", "disagreement")]
  rbd.model.comparison.list[[i]][, true_alt_mismatch := ifelse(gap == "gap", 
                                                               "identical", 
                                                               alt_mismatch)]
}
...
for (i in seq_along(ha.model.comparison.list)) {
  ha.model.comparison.list[[i]][, alt_mismatch := ifelse(.SD[[3]] == .SD[[4]] | 
                                                           .SD[[3]] == .SD[[8]], 
                                                         "identical", "disagreement")]
  ha.model.comparison.list[[i]][, true_alt_mismatch := ifelse(gap == "gap", 
                                                              "identical", 
                                                              alt_mismatch)]
}
...
summary.plausible.identity <- function(dt_list, model_pairs, model_names){
  ...
  # Pre-allocate output data table that will contain the data for all the pairwise
  # comparisons of each protein family
  model_comparison <- data.table(true_alt_mismatch = character(), 
                           count = integer(), 
                           percentage = numeric(),
                           model_a = character(),
                           model_b = character(),
  ...
  for(i in 1:length(dt_list)){
    ...
    # in the true_mismatch column. Percentages are relative to the total number of
    # reconstructions in the entire tree
    site_comparison <- site_comparison[, .(count = .N), by = true_alt_mismatch][,
      percentage := count/nrow(site_comparison)*100][]
  ...
  # Generate table with the self-comparisons between models. 
  self_comparisons <- data.table(true_alt_mismatch = "identical", 
                           count = nrow(dt_list[[1]]), 
                           percentage = 100,
                           model_a = model_names,
...
plot.pairwise.alt <- function(summarized_dt){
  ...
  # Only the percentages for identical reconstructions is plotted
  plot <- ggplot(summarized_dt[true_alt_mismatch == "identical"], aes(x = model_b, y = model_a)) +
...
ha_pars_file <- file.path(paste0(data.path, "/ha_parsimony_gaps.csv"))
rbd_pars_file <- file.path(paste0(data.path, "/rbd_parsimony_gaps.csv"))

# Use clean.gap.data() to organize information
sr.parsimony <- clean.pars.data(sr_pars_file)
ha.parsimony <- clean.gap.data(ha_pars_file)
rbd.parsimony <- clean.gap.data(rbd_pars_file)
...
for(i in 1:length(sr.model.comparison.list)){
  sr.model.comparison.list[[i]] <- merge(sr.model.comparison.list[[i]], 
                                         sr.parsimony, 
                                         by = c("node", "site"))
  sr.model.comparison.list[[i]][, true_pars_ambiguous := ifelse(gap == "gap", "unambiguous", pars_ambiguous)]
  sr.model.comparison.list[[i]][, c(1,2,3,4,8,6,7,10,12)]
}
...
for(i in 1:length(rbd.model.comparison.list)){
  rbd.model.comparison.list[[i]] <- merge(rbd.model.comparison.list[[i]], 
                                         rbd.parsimony, 
                                         by = c("node", "site"))
  rbd.model.comparison.list[[i]][, true_pars_ambiguous := ifelse(gap == "gap", "unambiguous", pars_ambiguous)]
  rbd.model.comparison.list[[i]][, c(1,2,3,4,8,6,7,10,12)]
}
...
for(i in 1:length(ha.model.comparison.list)){
  ha.model.comparison.list[[i]] <- merge(ha.model.comparison.list[[i]], 
                                         ha.parsimony, 
                                         by = c("node", "site"))
  ha.model.comparison.list[[i]][, true_pars_ambiguous := ifelse(gap == "gap", "unambiguous", pars_ambiguous)]
  ha.model.comparison.list[[i]][, c(1,2,3,4,8,6,7,10,12)]
}
...
read_parsimony_data <- function(node_directory, parsimony_file_path){
  ...
  parsimony_data <- parsimony %>%
    ...
    mutate(pars_ambiguous = ifelse(nchar(pars_state) > 1, "ambiguous", "unambiguous"))

  return(parsimony_data[, c("pars_ambiguous", "pars_state")])
}
...
for(i in seq_along(sr.model.comparison.list)) {
  table <- cbind(sr.model.comparison.list[[i]], sr_parsimony)
  table[, true_amb := ifelse(gap == "gap", "unambiguous", pars_ambiguous)]
  sr.model.comparison.list[[i]] <- cbind(table, 
                                         rates = sr_rates, 
                                         entropy = sr_entropy)
}
...
for(i in seq_along(ha.model.comparison.list)) {
  table <- cbind(ha.model.comparison.list[[i]], ha_parsimony)
  table[, true_amb := ifelse(gap == "gap", "unambiguous", pars_ambiguous)]
  ha.model.comparison.list[[i]] <- cbind(table, 
                                         rates = ha_rates, 
                                         entropy = ha_entropy)
}
...
for(i in seq_along(rbd.model.comparison.list)) {
  table <- cbind(rbd.model.comparison.list[[i]], rbd_parsimony)
  table[, true_amb := ifelse(gap == "gap", "unambiguous", pars_ambiguous)]
  rbd.model.comparison.list[[i]] <- cbind(table, 
                                         rates = rbd_rates, 
                                         entropy = rbd_entropy)
}
...
get_contingency_table <- function(table){
  ...
  contingency_table <- table %>%
    group_by(true_mismatch, pars_ambiguous) %>%
    tally() %>%
...
count(filter(sr.model.comparison.list[[2]], node %in% sr_nodes_with_dis), true_mismatch == "identical")

rbd_nodes_with_dis <- unique(filter(rbd_node_identity_binned, number_disagreements > 0)$node)
count(filter(rbd.model.comparison.list[[1]], node %in% rbd_nodes_with_dis), true_mismatch == "identical")


ha_nodes_with_dis <- unique(filter(ha_node_identity_binned, number_disagreements > 0)$node)
count(filter(ha.model.comparison.list[[2]], node %in% ha_nodes_with_dis), true_mismatch == "identical")
