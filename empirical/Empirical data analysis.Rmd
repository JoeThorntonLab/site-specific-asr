...
# causing an artifact in which model selection appears to be important. 
# artifact of the reconstruction algorithms. We performed parsimony based ASR to 
# identify sites that should be reconstructed as gaps using Mesquite 3.70 (Maddison 
# and Maddison 2021). We recoded the alignments to contain two states, G for sites
# with gaps, and A for sites with an amino acid states. As such, sites that should
# be reconstructed as gaps appear as Gs in the reconstructions along the tree.
...
clean_gap_data <- function(path){
  ...
  parsimony <- melt(parsimony, id.vars = "site", 
...
# Create a site vector from 1 to the alignment size
  ...
  for (i in seq_along(node_file_list)) {
    ...
    # Load sequences for both models
    model_1_sequence <- readLines(file.path(results.path, "sr.sh.jtt", "ASR", node_file_list[[i]]))
    model_2_sequence <- readLines(file.path(results.path, "sr.sh.poisson", "ASR", node_file_list[[i]]))
  ...
  merge(PP_tibble, HA_gaps, by = c("node", "site"))
